...
/*
    가상의 기계명령으로 구성된 프로그램을 실행하는 가상 기계를 작성하라.
    명령종류는 mov, add sub.jn0, prt, exi의 6가지이다.
    mov sum 0은 sum변수에 0을 저장하고. add sum i는 sum 변수에 i 변수의 값을 더하고
    add i 1은 변수 i를 1 증가시키며, sub n 1은 변수 n의 값을 1 감소시키고, jn0 n 3은 변수 n의 값이 0과 같이 않으면
    3번째 명령(실제 4번째 줄의 명령)으로 점프하도록 처리하며, prt sum은 sum 변수의 값을 화면에 출력하고, exi는 기계 명령으로 작성된
    사용자 프로그램을 종료하는 명령이다. go는 사용자가 입력한 기계 명령 프로그램을 처음부터 실행하도록 하는 지시어이다. prt와 exi를 제외한 명령들의 첫번째
    피연산자는 항상 변수이고, 두 번째 피연산자는 변수이거나 숫자가 될 수 있다.
*/
import java.util.*;
