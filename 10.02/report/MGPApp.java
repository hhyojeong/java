...
사람과 컴퓨터가 묵찌빠 게임을하는 프로그램을 만들어본다.
게임에서 주도권을 가진 선수를 오너(owner)라고 하며, 사람이 멈ㄴ저 오너가 된다.
사람은 사용자로부터 묵, 찌, 빠 중 하나를 입력받는다. 잘못된 입력이 들어온다면 다시 입력받는다.
컴퓨터는 묵, 찌, 바 중에서 랜덤하게 한 개를 선택하고 " 결정하였습니다 "를 출력한다.
사람이 오너인 경우 컴퓨터가 낸 것과 비교하여 같으면 사람이 승자가되고 프로그램은 종료한다. 컴퓨터가 오너인 경우도 마찬가지 이다\
승자가 결정되면 프로그램은 종료한다
사람과 컴퓨터가 낸 것이 승자가 없는 경우 가위바위보 규칙에 따라 이긴사람이 오너가 된다.
이 문제를 풀 때 힌트를 참고하여 사람과 컴퓨터의 공통 행동을 player 추상 클래스로 만들고 사람은 human 컴퓨터는 computer 클래스를 마들고 이들으 player를 상속받는 구조로 하라
